# z_ai_coding_streaming_fixed.py - Fixed version that preserves headers
from mitmproxy import http
import json
import time
from pathlib import Path

LOG_FILE = Path.home() / "zai-coding-streaming-fixed.log"
...
class ZaiCodingStreamingProxyFixed:
    def request(self, flow: http.HTTPFlow) -> None:
        original_path = flow.request.path
        
        # CRITICAL: Preserve headers BEFORE modifying request
        original_headers = dict(flow.request.headers)
        auth_header = flow.request.headers.get("Authorization") or flow.request.headers.get("authorization")
        ...
        # CRITICAL: Ensure Authorization header is preserved
        if auth_header:
            # Set both case variations to be safe
        ...
        _log({
            ...
            "authorization_before": _mask_auth(auth_header),
            "authorization_after": _mask_auth(flow.request.headers.get("Authorization")),
...
addons = [ZaiCodingStreamingProxyFixed()]
